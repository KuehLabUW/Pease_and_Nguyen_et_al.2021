...
for i= 1:numel(objects)
    ...
    for j= 1:numel(objects(i).obj)
        if objects(i).obj(j).gate == GateNumber
            if (objects(i).obj(j).data.logYFP > 1 && objects(i).obj(j).data.logYFP < 5)
...
tbinlow = linspace(0,180,21);
tbinhigh = tbinlow+8-eps;   % these are the well positions
...
for i = 1:length(tbinlow)
    ...
    [fitresult gof] = gauss2_2D(xrange, yrange, nry, param);
