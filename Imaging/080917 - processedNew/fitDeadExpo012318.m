function [fitresult, gof] = fitDeadExpo012318(xData, yData,S,A,d)

eq{1} = '(kd*%f)/(%f+%f)*exp(%f*x)+(yo-(kd*%f)/(%f+%f))*exp(-%f*x)';
eq{1} = sprintf(eq{1},A,S,d,S,A,S,d,d);
