...
well = 252;% Generate random well position from total number of wells 
...
for j = 1:15:100 % For images 1,41,81,121, and 161 || 1:40:177
    cd(['C:\Users\Sam Nguyen\Documents\MATLAB\Imaging\082117 - processed\pos ' num2str(well)]) % Enter folder for well position
    
    if j == 1
        im = load(['imgf_000' num2str(j) '.mat']); % Load image structure at j index
        img = double(im.images(ch).im); % Convert image to double
        cd('C:\Users\Sam Nguyen\Documents\MATLAB\Imaging\code\082117 EVp21cMyc\') % Enter folder with cellseg function
        [seg] = cellseg060117Correct(img,j); % Segment image
