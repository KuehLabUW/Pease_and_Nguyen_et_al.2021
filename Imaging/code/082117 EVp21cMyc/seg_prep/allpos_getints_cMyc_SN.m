...
for p = 394:527    % loop over all the stage positions
   pos(p) = load([dirname 'pos ' num2str(p) '/segtrackints.mat']);  % load all the objects
end

for p = 528:664    % loop over all the stage positions
   pos(p) = load([dirname 'pos ' num2str(p) '/segtrackints.mat']);  % load all the objects
end
...
for t = 1:T    % loop through all timepoints
    ...
    obj = [];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for p = 394:527   % loop through all stage positions
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for p = 528:664   % loop through all stage positions
        obj1 = pos(p).objects(t).obj;
        for i = 1:length(obj1)   % loop through all objects
            obj1(i).data.p = p;
            % log transform all the data points
