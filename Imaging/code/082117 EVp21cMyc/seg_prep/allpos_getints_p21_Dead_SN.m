...
for p = 256:393    % loop over all the stage positions
   pos(p) = load([dirname 'pos ' num2str(p) '/segtrackintsdead.mat']);  % load all the objects
end

for p = 665:795    % loop over all the stage positions
   pos(p) = load([dirname 'pos ' num2str(p) '/segtrackintsdead.mat']);  % load all the objects
end
...
for t = 1:T    % loop through all timepoints
    ...
    obj = [];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for p = 256:393   % loop through all stage positions
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for p = 665:795   % loop through all stage positions
        obj1 = pos(p).objects(t).obj;
        for i = 1:length(obj1)   % loop through all objects
            obj1(i).data.p = p;
            % log transform all the data points
...
gatenames = {'ALL'};
save([dirname 'segtracksint_Feb082117RatioDead.mat'],'objects','gatenames');
