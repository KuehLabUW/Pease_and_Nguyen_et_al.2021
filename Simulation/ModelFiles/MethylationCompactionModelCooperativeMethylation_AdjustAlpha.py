...
# Tellurium function
# Tellurium function
def doSimulation(CycleTime,alpha1,beta1,alpha2,lambda1,delta2):

    t = CycleTime

    # Run Tellurium
    r = loada("""
    R1: DC => MC ; (alpha1 + beta1*(MC+MUC))*DC
    R2: MC => DC ; alpha2*fraction*MC
    R3: DUC => MUC; (alpha1 + beta1*(MC+MUC))*DUC
...
#Write csv file
def writeCSV(ResultMatrix,AlphaON,BetaON,AlphaOFF,LambdaON,DeltaOFF,Cycle):
    with open('Trun50F0.825LifeTimePhaseAlphaON{}BetaON{}AlphaOFF{}LambdaON{}DeltaOFF{}Cycle{}.csv'.format(AlphaON,BetaON,AlphaOFF,LambdaON,DeltaOFF,Cycle),'w') as mycvsfile:
...
def generateLifeTime(repeat_number):
	    ...
	    BetaON  = betas[j]
            ...
            result = doSimulation(20,AlphaON,BetaON,AlphaOFF,LambdaON,DeltaOFF) #(CycleTime,alpha1,alpha2,lambda1,delta2,L)
...
RepeatArray = list(numpy.arange(1,6,1))
...
if __name__ == '__main__': #you can only run this from the command line
    ...
    for j in xrange(0,len(betas)): 
	    ...
	    AlphaON = 0.02
            ...
            writeCSV(U,AlphaON,betas[j],AlphaOFF,LambdaON,DeltaOFF,20)
            print 'Flag after'
