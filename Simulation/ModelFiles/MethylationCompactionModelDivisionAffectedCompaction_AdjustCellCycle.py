...
def doSimulation(CycleTime,alpha1,alpha2,lambda1,delta2):
    ...
    for i in xrange(0,20000/t,1): #simulate 20000/t cell cycle/total of 200000 points
        if i == 0:
            ...
            r.DUC = r.DUC + muc[0]
            W = r.DC
            dc = numpy.random.binomial(W,0.1,1)
            r.DC = r.DC - dc[0]
            r.DUC = r.DUC + dc[0]
            
            #for Methylated Compacted population do a multinomial distribution to the rest of the states
            Y = r.MC
            partition = numpy.random.multinomial(Y,[0.45,0.05,0.05,0.45])
            r.MC = partition[3]
            r.DC = r.DC + partition[0]
            r.MUC = r.MUC + partition[1]
            r.DUC = r.DUC + partition[2]
...
def writeCSV(ResultMatrix,AlphaON,AlphaOFF,LambdaON,DeltaOFF,Cycle):
    with open('Trun50F0.825LifeTimePhaseDivideCompAlphaON{}AlphaOFF{}LambdaON{}DeltaOFF{}Cycle{}.csv'.format(AlphaON,AlphaOFF,LambdaON,DeltaOFF,Cycle),'w') as mycvsfile:
...
def generateLifeTime(repeat_number):
            ...
            result = doSimulation(Cycles[j],AlphaON,AlphaOFF,LambdaON,DeltaOFF) #(CycleTime,alpha1,alpha2,lambda1,delta2,L)
...
if __name__ == '__main__': #you can only run this from the command line
    ...
    for j in xrange(0,len(Cycles)): 
            ...
            writeCSV(U,AlphaON,AlphaOFF,LambdaON,DeltaOFF,Cycles[j])
            print 'Flag after'
