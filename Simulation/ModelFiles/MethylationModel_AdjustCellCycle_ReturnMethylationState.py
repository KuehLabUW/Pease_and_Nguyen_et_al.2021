...
def doSimulation(CycleTime,alpha1,beta1,alpha2,beta2,L,TF,LT): #LT is the number of fixed nucleosome
    ...
    for i in xrange(0,20000/t,1): #simulate 5000/t cell cycle/total of 9990 points
        if i == 0:
            SubResult =r.simulate(0,t,int(t/0.1)) #each point is 2 in time
...
def writeCSV(ResultMatrix,alpha,beta,CellCycle):
    ...
    with open('RawAlpha{}Beta{}Cycle{}.csv'.format(alpha,beta,CellCycle),'w') as mycvsfile: #open up 'mydata.csv' for reading and pass it up to a variable
