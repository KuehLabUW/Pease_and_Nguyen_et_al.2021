...
#Write csv file
def writeCSV(ResultMatrix,alphaOFF,betaON,CellCycle,L):

    with open('LifeTimeAlpha{}Beta{}Cycle{}L{}.csv'.format(alphaOFF,betaON,CellCycle,L),'w') as mycvsfile: #open up 'mydata.csv' for reading and pass it up to a variable
...
def generateLifeTime(repeat_number):
            ...
            result = doSimulation(20,0,betaON,alphas[j],0,L,0,1) #(cell_cycle,alpha1,beta1,alpha2,beta2,L,TF,LT)#L was 15
...
if __name__ == '__main__': #you can only run this from the command line
    ...
    for j in xrange(0,len(alphas)): 
            ...
            #alphaOFF = 0
            #betaON = 0
            writeCSV(U,alphaOFF,betaON,20,L)
            print 'Flag after'; print np.mean(U)
